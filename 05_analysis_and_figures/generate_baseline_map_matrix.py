"""4-row × 4-column map matrix: domains × the four classical baselines.
Reads pre-computed regime labels from results_baselines/<ds>_labels.npz.
Outputs to paper_figures_baselines/.
"""
import os
import warnings
...
ALGOS = ["AZP", "RegionKModels", "GWR+SKATER", "SKATER-reg"]
ALGO_DISPLAY = {
    "AZP": "AZP", "RegionKModels": "Region-K-Models", "GWR+SKATER": "GWR-Skater",
    "SKATER-reg": "Skater-reg",
}
...
def load_labels(ds_name):
    ...
    coords = npz["coords"]
    labels_per_algo = {a: npz[a] for a in npz.files if a not in ("coords",)}
    return coords, labels_per_algo
...
def main():
    ...
    for ri, (domain, ds_name) in enumerate(REPRESENTATIVE):
        coords, labels_per_algo = load_labels(ds_name)
        if coords is None:
        ...
        for ci, algo in enumerate(ALGOS):
            ...
            labels = labels_per_algo.get(algo)
            if labels is None:
            ...
            if ci == 0:
                ax.annotate(domain, xy=(-0.05, 0.5), xycoords="axes fraction",
                            fontsize=8, fontweight="bold", rotation=90,
                            ha="right", va="center")
            cap = f"({sub[panel]}) {DS_DISPLAY[domain]} – {ALGO_DISPLAY[algo]} (k={n_reg})"
    ...
    for ext in ("pdf", "png"):
        out = os.path.join(OUT_DIR, f"baseline_regime_map_matrix.{ext}")
