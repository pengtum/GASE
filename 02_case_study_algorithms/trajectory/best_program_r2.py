...
def run_two_stage_kmeans(
    ...
    # Increase initial clusters adaptively; try 2.5*p but no more than 30 and not less than 2*p
    init_clusters = min(max(int(2.5*p), 2*p, 15), 30, X.shape[0] // max(min_size, 1))
    ...
    # 3. Local refinement step: reassign border units if it improves SSR without breaking connectivity
    # Increase refinement iterations and use a more robust batch border reassignment
    labels_refined = batch_border_refinement(X, Y, labels_merged, w, min_size, max_iter=20)
    return labels_refined

def batch_border_refinement(X, Y, labels, w, min_size, max_iter=10):
    """
    Batch reassignment of all possible border units to neighboring regions if it improves SSR and preserves connectivity.
    This is more robust and parallelizable than greedy single moves.
    """
    labels = labels.copy()
    for it in range(max_iter):
        ...
        for i in range(len(labels)):
            ...
            for r1 in neighbor_labels:
                # Try move i to r1 if both donor and recipient remain >= min_size and both remain connected
        ...
        for i, r1, gain in move_candidates:
            if i in moved:
                continue
            r0 = labels[i]
            # Double-check constraints as region_indices may have changed
...
if __name__ == '__main__':
    ...
    output = {
        "best_parameters": {
            "init_clusters": "adaptive (2.5*p, min 2*p, max 30)",
            "border_refinement_iters": 20,
            "init_scheme": "KMeans++/MiniBatchKMeans (spatially diverse)",
            "kmodels_border_moves": "SSR-improving, prioritized, batch border refinement"
        },
        "rand_index": float(randi),
        "ssr": float(ssr),
        "runtime": float(used_time),
        "spatial_contiguity_passed": True,
        "notes": "Adaptive initial clusters for fine start, batch border refinement for robust improvement, KMeans++ for diverse seeds, and SSR-improving moves in kmodels."
    }
    print(output)