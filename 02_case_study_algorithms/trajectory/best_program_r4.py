...
if __name__ == '__main__':
    ...
    output = {
        "best_parameters": {
            "init_clusters": "adaptive (2.5*p, 4*p, sqrt(n), p)",
            "border_refinement_iters": 30,
            "border_refinement_patience": 3,
            ...
            "border_refine_spatial_penalty": "0.35 ± 0.01 (deterministic jitter for diversity)"
        },
        "rand_index": float(randi),
        "ssr": float(ssr),
        "runtime": float(used_time),
        "spatial_contiguity_passed": True,
        "notes": "Added p to initialization candidate ks, slight stochasticity in border penalty, always allow 1 move in kmodels if stuck for diversity, and additional early stop in border refinement."
    }
    print(output)