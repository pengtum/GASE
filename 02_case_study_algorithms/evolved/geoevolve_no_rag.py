...
def run_two_stage_kmeans(
    ...
    """
    Two-stage k-means regionalization with improved initialization and merge.
    """
    if min_size is None:
        min_size = X.shape[1]

    # Use more oversegmentation and KMeans initialization for improved accuracy
    n_init_regions = max(24, 3*p)
    labels_partition = run_kmodels(
        X, Y,
        n_regions=n_init_regions,
        w=w,
        min_size=min_size,
        max_iter=max_iter,
        init_stoc_step=False,  # Force KMeans init for reproducibility and spatial fit
        verbose=verbose
    )
...
def run_kmodels(
    ...
    """
    'k-models' like logic (stage 1), improved for stability and accuracy.
    """
    if min_size is None:
        min_size = X.shape[1]

    units = np.arange(w.n).astype(int)
    # Use KMeans init if possible for accuracy
    labels = init_zones(w.n, n_regions, X, stoc_step=not init_stoc_step if X is not None else True)

    prev_labels = labels.copy()
    for iters in range(max_iter):
        regions = [units[labels == r].tolist() for r in range(n_regions)]
        # Ridge only if n_regions > 15 for numerical stability
        ridge = 0.1 if n_regions > 15 else 0.0
        coeffs = fit_equations(X, Y, regions, ridge_alpha=ridge)
...
def init_zones(n, k, X=None, stoc_step=True):
    """
    Initialize zones: KMeans if X and not stoc_step, else random.
    """
    if X is not None and not stoc_step:
        km = KMeans(n_clusters=k, random_state=42, n_init=5, max_iter=100)
        return km.fit_predict(X)
    else:
        return np.random.randint(0, k, size=n)

def fit_equations(X, Y, regions, ridge_alpha=0.0):
    """
    Fit regression for each region, optionally using ridge regression for stability.
    ...
    for r_units in regions:
        ...
        if ridge_alpha > 0:
            ...
            XtY = Xr.T @ Yr
            try:
                beta_r = np.linalg.solve(XtX + ridge_alpha * np.eye(Xr.shape[1]), XtY)
...
def closest_equation(X, Y, coeffs):
    """
    For each observation in X, compute SSR to each region's coefficients.
    Return best-fit region index.
    """
    n = len(Y)
    k = len(coeffs)
    pred = np.full((n, k), np.inf)
    for j in range(k):
        if coeffs[j] is None:
            continue
        pred[:, j] = (Y - X.dot(coeffs[j]))**2
...
def merge_stage(X, Y, labels, w, p, min_size, verbose=False):
    ...
    labels_no_small = relabel_sequential(labels_no_small)
    nreg = len(np.unique(labels_no_small))
    if nreg > p:
        labels_final = greedy_merge_until_p(X, Y, labels_no_small, w, p, verbose)
    else:
        labels_final = labels_no_small
    labels_final = relabel_sequential(labels_final)
    return labels_final
...
def compute_ssr_for_units(X, Y, units, ridge_alpha=0.0):
    """Compute SSR for given units by OLS or ridge fit."""
    if len(units) == 0:
        return 0.0
    Xr = X[units, :]
    Yr = Y[units]
    if ridge_alpha > 0:
        ...
        XtY = Xr.T @ Yr
        try:
            beta = np.linalg.solve(XtX + ridge_alpha * np.eye(Xr.shape[1]), XtY)
        except np.linalg.LinAlgError:
...
def fast_hierarchical_merge(X, Y, labels, w, p, verbose=False):
    """
    Agglomerative region merging using region adjacency and SSR loss,
    optimized for speed when there are many small regions.
    """
    # Build initial regions and adjacency
    unique_labels = np.unique(labels)
    region_units = {r: set(np.where(labels == r)[0]) for r in unique_labels}
    region_neighbors = build_region_adjacency(labels, unique_labels, w)
    region_coeffs = {r: fit_equations(X, Y, [list(region_units[r])], ridge_alpha=1e-4)[0] for r in unique_labels}
    region_ssr = {r: compute_region_ssr(X, Y, list(region_units[r]), region_coeffs[r]) for r in unique_labels}

    import heapq
    merge_heap = []
    # For each neighbor pair, compute potential SSR decrease
    for rA in region_units:
        for rB in region_neighbors[rA]:
            if rA < rB:
                unitsA = region_units[rA]
                unitsB = region_units[rB]
                merged_units = list(unitsA | unitsB)
                ssrA = region_ssr[rA]
                ssrB = region_ssr[rB]
                ssrMerged = compute_ssr_for_units(X, Y, merged_units, ridge_alpha=1e-4)
                delta = (ssrA + ssrB) - ssrMerged
                # Use negative delta since heapq is minheap
                heapq.heappush(merge_heap, (-delta, rA, rB))

    label_parent = {r: r for r in region_units}

    def find_root(r):
        while label_parent[r] != r:
            r = label_parent[r]
        return r

    n_regions = len(region_units)
    while n_regions > p and merge_heap:
        neg_delta, rA, rB = heapq.heappop(merge_heap)
        rA = find_root(rA)
        rB = find_root(rB)
        if rA == rB or rA not in region_units or rB not in region_units:
            continue
        # Merge B into A
        unitsA = region_units[rA]
        unitsB = region_units[rB]
        merged_units = unitsA | unitsB
        region_units[rA] = merged_units
        del region_units[rB]
        label_parent[rB] = rA
        # Update adjacency
        region_neighbors[rA].update(region_neighbors[rB])
        region_neighbors[rA].discard(rA)
        region_neighbors[rA].discard(rB)
        for nb in region_neighbors[rB]:
            if nb != rA and nb in region_neighbors:
                region_neighbors[nb].discard(rB)
                region_neighbors[nb].add(rA)
        del region_neighbors[rB]
        # Update SSR and coeffs
        region_coeffs[rA] = fit_equations(X, Y, [list(merged_units)], ridge_alpha=1e-4)[0]
        region_ssr[rA] = compute_region_ssr(X, Y, list(merged_units), region_coeffs[rA])
        del region_coeffs[rB]
        del region_ssr[rB]
        # Add new merge candidates
        for rC in region_neighbors[rA]:
            if rC == rA or rC not in region_units:
                continue
            merged2 = merged_units | region_units[rC]
            ssrA2 = region_ssr[rA]
            ssrC = region_ssr[rC]
            ssrMerged2 = compute_ssr_for_units(X, Y, list(merged2), ridge_alpha=1e-4)
            delta2 = (ssrA2 + ssrC) - ssrMerged2
            heapq.heappush(merge_heap, (-delta2, rA, rC))
        n_regions = len(region_units)

    # Assign output labels
    out_labels = np.zeros_like(labels)
    label_map = {}
    for idx, (r, units) in enumerate(region_units.items()):
        for u in units:
            out_labels[u] = idx
        label_map[r] = idx
    return out_labels
...
def plot_results(true_label, results, titles, side=25, name=None):
    ...
    plt.savefig(f'{titles[0]}_{name}_geoevolve_no_rag.pdf')
