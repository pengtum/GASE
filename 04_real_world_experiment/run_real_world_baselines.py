"""Real-world dataset analysis with classical spatial-regime / regionalization
baselines + the best evolved GeoEvolve model.

Algorithms compared:
  - AZP                (regression-AZP)
  - RegionKModels      (region-k-models)
  - GWR+SKATER         (Geographically Weighted Regression coefficients clustered with SKATER)
  - SKATER-reg         (regression-objective SKATER)
  - SHAP-based         (XGBoost + SHAP + spatial cluster + k-models refinement)
  - GeoEvolve          (best evolved 2k-models — for reference)

Output:
  results_baselines/
    algorithm_comparison.csv
    <ds>_<algo>_labels.npy
    results_summary.pkl
"""
...
OUTPUT_DIR = os.path.join(BASE, "results_baselines")
os.makedirs(OUTPUT_DIR, exist_ok=True)

# Make codes/baselines importable
...
GEOEVOLVE_FILE = os.path.join(BASE, "best_program_geoevolve.py")

ALGORITHMS = {
    "AZP": (azp.run, dict()),
    "RegionKModels": (region_k_models.run, dict()),
    "GWR+SKATER": (gwr_skater.run, dict()),
    "SKATER-reg": (skater_reg.run, dict()),
    "SHAP-based": (shap_based.run, dict()),
}
...
def load_geoevolve_module():
    spec = importlib.util.spec_from_file_location("geoevolve", GEOEVOLVE_FILE)
...
GEOEVOLVE = None
try:
    GEOEVOLVE = load_geoevolve_module()
    print(f"Loaded GeoEvolve: {GEOEVOLVE_FILE}")
except Exception as e:
    print(f"Warning: could not load GeoEvolve module: {e}")
...
def compute_summary(X, Y, labels):
    """Return dict with global/regime SSR, R^2, n_regions, plus per-region intra dict."""
    intra = {}
    total_ssr = 0.0
    for r in np.unique(labels):
        ...
        n = len(Yr)
        if n < X.shape[1]:
            intra[r] = {"n": n, "beta": None, "y_mean": float(np.mean(Yr)) if n > 0 else np.nan,
        ...
        intra[r] = {"n": n, "beta": beta, "y_mean": float(np.mean(Yr)),
...
def call_algo(name, X, Y, p, w, coords):
    if name == "GeoEvolve":
        if GEOEVOLVE is None:
            raise RuntimeError("GeoEvolve module not loaded")
        min_size = max(X.shape[1], 10)
        return GEOEVOLVE.run_two_stage_kmeans(
            X, Y, p=p, w=w, min_size=min_size, max_iter=300,
            init_stoc_step=True, verbose=False)
    fn, _ = ALGORITHMS[name]
    kwargs = dict(coords=coords) if name in NEEDS_COORDS else {}
    min_size = max(X.shape[1], 10)
    return fn(X, Y, n_regions=p, w=w, min_size=min_size, max_iter=200,
              seed=42, **kwargs)
...
def main():
    ...
    if GEOEVOLVE is not None:
        algo_names.append("GeoEvolve")
    ...
    for f in csv_files:
        ...
        try:
            ...
            print(f"  shape X={X.shape}, Y={Y.shape}, features={len(xcols)}")
            w = build_w(coords)
            p = determine_p(regions_orig)
            print(f"  p={p}")
            coords_per_ds[ds] = coords

            ds_results = {}
            for algo in algo_names:
                t0 = time.time()
                try:
                    labels = call_algo(algo, X, Y, p, w, coords)
            ...
            saved_results[ds] = ds_results
            # Persist after each dataset (resumable)
    ...
    # Final summary pickle
    to_pickle = {}
    for ds, dr in saved_results.items():
        to_pickle[ds] = {algo: {"runtime": ar["runtime"], "summary": ar["summary"]}
