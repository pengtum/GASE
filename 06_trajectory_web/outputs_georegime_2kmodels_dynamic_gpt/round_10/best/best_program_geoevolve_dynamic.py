...
def run_geogregime_different_schemes(base_path = '<PROJECT_ROOT>/examples/georegime/regreg/synthetic',
    ...
    for id_ in data_ids:
        ...
        if randi < worst_case_randi:
            worst_case_randi = randi
            worst_case_id = id_
        print(f'ssr: {ssr}, randi: {randi}, nmi: {nmi}, mae: {mae}')
    return np.mean(ssr_list), np.mean(randi_list), np.mean(nmi_list), np.mean(mae_list), np.mean(used_time_list), worst_case_id
